...
import { Search, X, Check, Calendar, Download, RotateCcw, Upload } from 'lucide-react';
...
const CompanyRenewal = () => {
    ...
    const baseDocs = documents.filter(doc => doc.category === 'Company' && doc.needsRenewal && doc.renewalDate);
    ...
    const handleDownload = (fileContent: string | undefined, fileName: string | null) => {
        if (!fileContent && (!fileName || !fileName.startsWith('http'))) {
            setAlertMessage("Document file is not available in the database.");
            setShowAlert(true);
            return;
        }
        ...
        <div className="space-y-4 font-sans">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                <div className="relative flex-1 w-full max-w-lg">
                    ...
                    <input
                        type="text"
                        placeholder="SEARCH COMPANY RENEWALS..."
                            ...
                            {displayDocs.length > 0 ? displayDocs.map((doc) => {
                                ...
                                return (
                                    <tr key={doc.id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-5 py-3 text-gray-600 text-[10px] font-bold uppercase">{doc.documentType}</td>
                                        <td className="px-5 py-3 text-gray-900 font-medium">{doc.companyName}</td>
                                        <td className="px-5 py-3 text-center">
            ...
            <div className="md:hidden space-y-4">
                {displayDocs.length > 0 ? displayDocs.map((doc) => {
                        ...
                        <div key={doc.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 space-y-3">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <p className="text-[10px] text-gray-500 font-bold uppercase mt-0.5">ID: {doc.sn}</p>
                                </div>
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                            ...
                            <div className="flex justify-between items-center text-xs pt-2 border-t border-gray-50">
                                <div>
                                    ...
                                    <p className="text-gray-500 mt-0.5">Renew: {doc.renewalDate ? formatDate(doc.renewalDate) : '-'}</p>
                                </div>
                    ...
                    <div className="py-10 text-center text-gray-400">
                        ...
                        <p className="text-sm">No renewals required</p>
                    </div>
                )}
            </div>
                        ...
                        <form onSubmit={handleSaveRenewal} className="p-6 space-y-4">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    <span className="text-sm font-medium text-gray-700">Set future renewal?</span>
                                    <button 
                                        type="button"
                                ...
                                {againRenewal && (
                                    <div className="space-y-4 animate-fade-in">
                                        <div>
                                            <label className="block text-[10px] font-black text-gray-900 uppercase tracking-widest mb-1">Next Renewal Date</label>
                                            <input 
                                                type="date" 
                                                required
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-black text-gray-900 uppercase tracking-widest mb-1">Update File</label>
                            ...
                            <div className="flex gap-3 pt-4">
                                <button type="button" onClick={handleCloseRenewal} className="flex-1 px-4 py-2 border border-gray-200 rounded-lg text-sm font-bold text-gray-600 hover:bg-gray-50">
                                    Cancel
                                </button>
