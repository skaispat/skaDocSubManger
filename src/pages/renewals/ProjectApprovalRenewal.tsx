...
import { Search, CheckCircle, X, Check, Calendar, RotateCcw, Upload, Eye, FileText, History, Download } from 'lucide-react';
...
const ProjectApprovalRenewal = () => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const data = await documentService.getAll('project_approval');
            const filtered = data.filter(item =>
                item.renewable?.toString().toLowerCase() === 'yes'
            );
            setProjectDocs(filtered);
        } catch (error) {
            toast.error("Failed to sync project approvals");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleViewHistory = async (doc: any) => {
        ...
        setIsLoading(true);
        try {
            const history = await documentService.getRenewalHistory('renew_project', doc.id_no);
    ...
    const handleDownload = (fileUrl: string | undefined, fileName: string | null) => {
        if (!fileUrl) {
            setAlertMessage("Approval document file is not available.");
            setShowAlert(true);
            return;
        }
    ...
    const handleSaveRenewal = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 1. Log to history table: renew_project
            const renewalData = {
            ...
            await documentService.logRenewal('renew_project', renewalData);
            ...
            const success = await documentService.update('project_approval', selectedDoc.id_no, updates);

            if (success) {
                toast.success("Project history logged and record updated");
                fetchData();
                handleCloseRenewal();
            } else {
            ...
            console.error("Project Renewal Error:", error);
        ...
        <div className="space-y-4 font-sans">
            ...
            <div className="flex flex-col gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div className="min-h-[32px] flex items-center">
                        <h2 className="text-sm sm:text-base font-bold text-gray-800 uppercase tracking-widest">Project Renewals</h2>
                    </div>
                    ...
                    <div className="flex bg-gray-50 p-1.5 rounded-lg border border-gray-200 w-full sm:w-auto">
                        ...
                        >
                            All <span className="ml-1 opacity-40">{projectDocs.length}</span>
                        </button>
                        <button
                ...
                <div className="relative w-full">
                    ...
                    <input
                        type="text"
                        placeholder="SEARCH ACROSS PROJECT RENEWALS..."
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {isLoading ? (
                                <tr key="loading-desktop"><td colSpan={5} className="p-20 text-center text-gray-400">Syncing data...</td></tr>
                                ...
                                return (
                                    <tr key={doc.id_no || `proj-${index}`} className="hover:bg-gray-50/50 transition-colors">
                                ...
                                <tr key="empty-desktop">
                                    <td colSpan={5} className="p-20 text-center text-gray-400">
                                        ...
                                        <p className="font-medium">All project approvals are current</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
            ...
            <div className="md:hidden space-y-4">
                {isLoading ? (
                    ...
                    return (
                        <div key={doc.id_no || `proj-mobile-${index}`} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-3">
                    ...
                    <div key="empty-mobile" className="py-10 text-center text-gray-400">
                        ...
                        <p className="text-xs">No project renewals required</p>
                    </div>
                )}
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-scale-in">
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                            <h3 className="text-lg font-bold text-gray-900">Renew Project Approval</h3>
                        ...
                        <form onSubmit={handleSaveRenewal} className="p-6 space-y-4">
                            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 text-[11px] space-y-2">
                                <div className="flex justify-between">
                                    <span className="text-gray-700 font-bold uppercase tracking-wider">Project:</span>
                                    <span className="font-black text-gray-950">{selectedDoc.document_name}</span>
                                </div>
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                <button type="submit" className="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-bold hover:bg-red-700 transition-all">Process</button>
                            </div>
                        </form>
                    </div>
                </div>
...
export default ProjectApprovalRenewal;
