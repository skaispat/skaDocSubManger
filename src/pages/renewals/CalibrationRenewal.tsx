...
import { Search, ShieldCheck, X, Check, Calendar, RotateCcw, Upload, Eye, FileText, History, CheckCircle, Download } from 'lucide-react';
...
const CalibrationRenewal = ({ navigator }: { navigator?: React.ReactNode }) => {
    ...
    const [calibrationDocs, setCalibrationDocs] = useState<any[]>([]);
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const data = await documentService.getAll('calibration_certificate');
            const filtered = data.filter(item =>
                item.renewable?.toString().toLowerCase() === 'yes'
            );
            setCalibrationDocs(filtered);
        } catch (error) {
            toast.error("Failed to sync calibration records");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const baseDocs = calibrationDocs;

    const searchedDocs = baseDocs.filter(doc =>
        (doc.instrument_name?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
        (doc.brand_name?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
    ...
    const handleViewHistory = async (doc: any) => {
        ...
        setIsLoading(true);
        try {
            const history = await documentService.getRenewalHistory('renew_calibration_certi', doc.id_no);
    ...
    const handleDownload = (fileUrl: string | undefined, fileName: string | null) => {
        if (!fileUrl) {
            setAlertMessage("Calibration certificate file is not available.");
            setShowAlert(true);
            return;
        }
    ...
    const handleSaveRenewal = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 1. Log to history table: renew_calibration_certi
            const renewalData = {
                doc_id: selectedDoc.id_no,
                instrument_name: selectedDoc.instrument_name,
                document_view: finalDocView,
                brand_name: selectedDoc.brand_name,
                id_sr_no: selectedDoc.id_sr_no,
                location: selectedDoc.location,
                certificate_number: selectedDoc.certificate_number,
                last_renewable_date: selectedDoc.renewable_date,
                renewable_date: againRenewal ? nextRenewalDate : null,
                calibration_date: new Date().toISOString().split('T')[0], // Today as calibration date
                last_calibration_date: selectedDoc.calibration_date,
                validity_period: validityPeriod,
                created_at: new Date().toISOString()
            };

            await documentService.logRenewal('renew_calibration_certi', renewalData);
            ...
            const success = await documentService.update('calibration_certificate', selectedDoc.id_no, updates);
            if (success) {
                toast.success("Calibration history logged and record updated");
                fetchData();
                handleCloseRenewal();
            } else {
            ...
            console.error("Calibration Renewal Error:", error);
        ...
        <div className="space-y-4 font-sans">
            ...
            <div className="md:hidden space-y-4">
                {displayDocs.length > 0 ? displayDocs.map((doc, index) => {
                    ...
                    const isCritical = !isOverdue && doc.renewable_date && new Date(doc.renewable_date) <= criticalThreshold;
                    return (
                        <div key={doc.id_no || `cal-mobile-${index}`} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-3">
                            <div className="flex justify-between items-start">
                                <div><h3 className="font-bold text-gray-900">{doc.instrument_name}</h3></div>
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${isOverdue ? 'text-red-700 bg-red-50' : isCritical ? 'text-amber-700 bg-amber-50' : 'text-green-700 bg-green-50'}`}>{isOverdue ? 'Overdue' : isCritical ? 'Critical' : 'Healthy'}</span>
                            </div>
                            <div className="space-y-3 pt-3 border-t border-gray-50 font-sans">
                                <div className="flex justify-between items-center text-xs">
                                    <div>
                                        <p className="font-black text-gray-400 uppercase tracking-tighter text-[9px]">Due Date</p>
                ...
                }) : (<div key="empty-mobile" className="py-10 text-center text-gray-400"><ShieldCheck size={40} className="mx-auto mb-2 text-green-100" /><p className="text-xs">No calibration renewals required</p></div>)}
            </div>

            {isRenewalModalOpen && selectedDoc && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-scale-in">
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50"><h3 className="text-lg font-bold text-gray-900">Renew Calibration</h3><button onClick={handleCloseRenewal} className="text-gray-400 hover:text-gray-600"><X size={20} /></button></div>
                        <form onSubmit={handleSaveRenewal} className="p-6 space-y-4">
                            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 text-[11px] space-y-2">
                                <div className="flex justify-between">
                                    <span className="text-gray-700 font-bold uppercase tracking-wider">Equipment:</span>
                                    <span className="font-black text-gray-900">{selectedDoc.instrument_name}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-700 font-bold uppercase tracking-wider">Brand/Asset:</span>
                                    <span className="text-gray-950 font-black">{selectedDoc.brand_name}</span>
                                </div>
                                <div className="flex justify-between border-t border-gray-100 pt-2 mt-2">
                                    <span className="text-gray-700 font-bold uppercase tracking-wider">Expiry Date:</span>
                                    <span className="font-black text-red-600">{selectedDoc.renewable_date ? formatDate(selectedDoc.renewable_date) : 'URGENT'}</span>
                                </div>
                            </div>
                            <div className="space-y-4">
                                <div className="flex items-center justify-between"><span className="text-sm font-medium text-gray-700">Schedule next?</span><button type="button" onClick={() => setAgainRenewal(!againRenewal)} className={`w-10 h-5 flex items-center p-1 rounded-full transition-colors ${againRenewal ? 'bg-red-600' : 'bg-gray-300'}`}><div className={`w-3 h-3 bg-white rounded-full transform transition-transform ${againRenewal ? 'translate-x-5' : 'translate-x-0'}`} /></button></div>
                                {againRenewal && (
                                    <div className="space-y-4 animate-fade-in">
                                        <div>
                                            <label className="block text-[11px] font-black text-gray-700 uppercase tracking-wider mb-1">Next Renewal Date</label>
                                            <input type="date" required className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-red-100 font-bold text-gray-950" value={nextRenewalDate} onChange={e => setNextRenewalDate(e.target.value)} />
                                        </div>
                                        <div>
                                            <label className="block text-[11px] font-black text-gray-700 uppercase tracking-wider mb-1">Validity Period</label>
                                            <input type="text" placeholder="e.g. 1 Year, 6 Months" className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-red-100 font-bold text-gray-950" value={validityPeriod} onChange={e => setValidityPeriod(e.target.value)} />
                                        </div>
                                        <div>
                            ...
                            <div className="flex gap-3 pt-4"><button type="button" onClick={handleCloseRenewal} className="flex-1 px-4 py-2 border border-gray-200 rounded-lg text-sm font-bold text-gray-600 hover:bg-gray-50 transition-colors">Cancel</button><button type="submit" className="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-bold hover:bg-red-700 transition-all">Process</button></div>
                        </form>
                    </div>
                </div>
            )}
