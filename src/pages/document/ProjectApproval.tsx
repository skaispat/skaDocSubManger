...
const ProjectApproval = ({ navigator }: { navigator?: React.ReactNode }) => {
    ...
    return (
        <>
                        ...
                        {selectedIds.size > 0 ? (
                            <div className="flex flex-wrap items-center gap-3 animate-fade-in w-full sm:w-auto">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        onClick={() => openShare('email', { id: 'batch', name: `${selectedIds.size} Approvals` })}
                                    ...
                                    <button
                                        onClick={() => openShare('whatsapp', { id: 'batch', name: `${selectedIds.size} Approvals` })}
                            ...
                            <>
                                <h2 className="text-md font-black text-gray-900 uppercase tracking-tight">Project Approvals</h2>
                                {navigator}
                            </>
                        )}
                    </div>
                        ...
                        <div key={item.id || `proj-mobile-${index}`} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-3">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <p className="text-[10px] text-gray-500 font-bold uppercase mt-0.5">Project Doc</p>
                                </div>
                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${
                                    item.status === 'Completed' || item.status === 'Active' ? 'bg-green-50 text-green-700 border-green-100' : 
                                    item.status === 'Pending' ? 'bg-yellow-50 text-yellow-700 border-yellow-100' : 
            ...
            <AddDocument isOpen={isAddModalOpen} onClose={() => { setIsAddModalOpen(false); fetchData(); }} initialCategory="Project" lockCategory={true} />
            ...
            <ConfirmModal
                ...
                message="Are you sure you want to delete this project approval record?"
                confirmText="Delete"
                type="confirm"
            />
            <ConfirmModal
