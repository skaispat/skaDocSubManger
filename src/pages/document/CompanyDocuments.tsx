...
const AllDocuments = ({ navigator }: { navigator?: React.ReactNode }) => {
    ...
    const confirmDelete = async () => {
        if (docToDelete) {
            const success = await documentService.delete('company_documents', docToDelete);
            if (success) {
    ...
    return (
        <>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                    <div className="min-h-[40px] flex items-center justify-between w-full sm:w-auto gap-4">
                        {selectedIds.size > 0 ? (
                            ...
                            <>
                                <h2 className="text-md font-black text-gray-900 uppercase tracking-tight">Company Documents</h2>
                                {navigator}
                            </>
                        )}
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-red-50 shadow-sm text-[10px] md:text-[11px] uppercase font-black tracking-widest text-gray-950">
                                    ...
                                    <th className="px-4 py-4 text-center">Renewal</th>
            ...
            <AddDocument isOpen={isAddModalOpen} onClose={() => { setIsAddModalOpen(false); fetchData(); }} initialCategory="Company" lockCategory={true} />
