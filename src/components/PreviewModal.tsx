...
const PreviewModal: React.FC<PreviewModalProps> = ({ isOpen, onClose, files, documentName }) => {
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 md:p-8 animate-in fade-in duration-200 font-sans">
            <div className="relative w-full h-full max-w-7xl bg-white rounded-3xl overflow-hidden shadow-2xl flex flex-col">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/50">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-sm font-black text-gray-900 uppercase tracking-tight truncate max-w-[200px] md:max-w-md">
                                {documentName}
                            </h2>
                            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-0.5">
                                File {currentIndex + 1} of {files.length} • {isPdf ? 'PDF Document' : 'Image'}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-gray-900 hover:bg-black text-white rounded-xl text-[10px] font-black uppercase tracking-wider transition-all disabled:opacity-50"
                        >
                            <Download size={14} />
                        ...
                        <button
                            ...
                            className="p-2 text-gray-400 hover:text-blue-600 transition-colors bg-white border border-gray-200 rounded-xl"
                        ...
                        <button
                            onClick={onClose}
                            className="p-2 text-gray-400 hover:text-red-600 transition-colors bg-white border border-gray-200 rounded-xl ml-2"
                        >
                            <X size={18} />
                        </button>
                    </div>
                                ...
                                {files.map((url, idx) => (
                                    ...
                                    >
                                        ...
                                        <div className={`p-2 text-left bg-white transition-colors ${currentIndex === idx ? 'bg-red-50/50' : ''}`}>
                                            <p className={`text-[10px] font-bold truncate ${currentIndex === idx ? 'text-red-700' : 'text-gray-500'}`}>
                                                File {idx + 1}
                                            </p>
                                        </div>
                                    </button>
